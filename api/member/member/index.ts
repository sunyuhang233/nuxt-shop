...
export async function login(data: LoginParam) {
  ...
  const res = await request.post<LoginResult>('/member/member/login', {...data}, {server: false});
...
export async function getMemberInfo(): Promise<Member> {
  const res = await request.get<Member>('/member/member/auth/member', null, {server: true});
  if (res.code === 0 && res.data) {
    return res.data;
  }
  return Promise.reject(new Error(res.message));
