...
<template>
  ...
  >
    <h3 style="margin-left: 20px; margin-top: 18px;">{{ paymentResult.paymentType === 'alipay' ? '扫码支付' : '确认会员订单' }}</h3>
    <van-divider />
    <div style="margin: 20px">
      ...
      <div class="weui-form-preview__bds">
        ...
        <div class="weui-form-preview__items">
          <label class="weui-form-preview__labels">会员价格</label>
          <span class="weui-form-preview__values">{{ agentPackage.price?.toFixed(2) }}</span>
        </div>
        <div class="weui-form-preview__items">
      ...
      <div class="bomaos-row">
        ...
        <n-button
            ...
            style="flex-shrink: inherit"
        >
          {{ paymentResult.paymentType === 'alipay' ? '拉起支付宝APP支付' : '立即前往支付' }}
        </n-button>
      </div>
    </div>
  </van-popup>
