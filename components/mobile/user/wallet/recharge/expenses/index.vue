<template>
  <div v-if="state" style="margin: 15px">
    <mobile-common-empty description="暂无消费记录" height="calc(100vh - 415px)">
      <template #icon>
  ...
  <div v-else>
    <div class="bomaos-wrapper">
      <n-row :gutter="[15, 0]">
        <n-col :span="24" class="van-clearfix" v-for="(item, index) in list" :key="item.id">
          <div class="bomaos-order-item" :style="{borderTop: index == 0 ? 'none' : '1px solid rgb(238 240 242)'}">
            <div class="bomaos-order-detail">
              <div class="bomaos-order-title">
                <h3>{{ item.name }}</h3>
                <!--              <span>
                                {{ item.recordType == 1 ? '收入' : '消费' }}
                              </span> &lt;!&ndash; 佣金 &ndash;&gt;-->
              </div>
              <div class="bomaos-order-bottom">
                <h4>付款金额: {{ item.money }}</h4>
                <span>余额: {{ item.balance }}</span>
              </div>
              <div class="paytime">消费时间: {{ item.createTime }}</div>
            </div>
          </div>
        </n-col>
      </n-row>
...
const query = (limit?: number, page?: number) => {
  loading.value = true;
  setTimeout(() => {
    pageMoneyRecords({limit, page}).then((res) => {
      if (res?.count === 0) {
        state.value = true;
        loading.value = false;
        return;
